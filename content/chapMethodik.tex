...
Die eine Variante steht für maximale Kompression bei minimaler Methodik (naive Binarisierung), die andere für moderate Kompression bei informierter Methodik (TurboQuant).
%
...
Der Effekt der Bitreduktion selbst wird am Unterschied zwischen Float und TurboQuant sichtbar, der Effekt fehlender Vorverarbeitung am Unterschied zwischen TurboQuant und naiver Binarisierung.
%
...
Für jedes Paar werden drei Distanzwerte berechnet, nämlich die Cosine-Similarity im Float-Raum, die Hamming-Distanz im binären Raum und die entsprechende Distanz im TurboQuant-Raum.
%
...
Die in den vorherigen Phasen untersuchten Retrieval-Backends (Float, binär, TurboQuant) werden als MCP-Server implementiert und von einem LLM-Agenten über das Model Context Protocol (vgl. Kapitel~\ref{sec:grundlagen_mcp}) als Retrieval-Werkzeug genutzt.
%
...
\begin{itemize}
  \item Drei identische MCP-Server, die sich ausschließlich im Retrieval-Backend unterscheiden (Float/Cosine, Binär/Hamming, TurboQuant).
...
Der Agent, das Prompt-Template und die Dokumentenbasis bleiben über alle drei Varianten konstant.
%
...
Für jede Query wird der Agent mit jedem der drei Backends ausgeführt.
Gemessen werden:
\begin{itemize}
