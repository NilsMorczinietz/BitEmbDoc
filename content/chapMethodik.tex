...
%
Aus diesem Float-Raum werden drei komprimierte Varianten \emph{nachträglich} abgeleitet.
...
\begin{enumerate}
  ...
  \item Sie isoliert genau eine unabhängige Variable, nämlich das Kompressionsverfahren. Bei end-to-end trainierten Modellen wären Unterschiede in der Retrieval-Qualität nicht eindeutig auf die Kompression zurückführbar, sondern könnten ebenso an Architektur, Trainingsdaten oder Verlustfunktion liegen.
...
Die naive Binarisierung steht für maximale Kompression bei minimaler Methodik, TurboQuant 2-Bit für starke Kompression mit informierter Vorverarbeitung, und TurboQuant 4-Bit für moderate Kompression bei hoher erwarteter Qualitätserhaltung.
%
...
%
Die zweite und dritte Variante nutzen das TurboQuant-Verfahren \cite{zandieh_2025_turboquant}, das eine datenunabhängige Rotation mit anschließender skalarer Quantisierung kombiniert.
Das Verfahren wird in zwei Bitstiefen evaluiert:
\begin{itemize}
  \item \textbf{TurboQuant 2-Bit} (4 Quantisierungsstufen pro Dimension, Kompressionsfaktor 16).
  \item \textbf{TurboQuant 4-Bit} (16 Quantisierungsstufen pro Dimension, Kompressionsfaktor 8).
\end{itemize}
%
Die 4-Bit-Variante schließt eine Lücke im Kompression-Qualität-Spektrum zwischen Float32 und 2-Bit.
...
Zeigt die 4-Bit-Variante nur marginale Verbesserung gegenüber 2-Bit, liegt der Qualitätsverlust vor allem in der Rotation und nicht in der Auflösung.
Zeigt sie hingegen deutlich bessere Ergebnisse, ist die Stufenzahl der dominierende Faktor.
%
...
Der Effekt der Bitreduktion wird am Unterschied zwischen Float und TurboQuant 4-Bit sichtbar, der Effekt extremer Kompression am Unterschied zwischen 4-Bit und 2-Bit, und der Effekt fehlender Vorverarbeitung am Unterschied zwischen TurboQuant 2-Bit und naiver Binarisierung.
%
...
\begin{itemize}
  \item Bittiefe: Float32, 4~Bit (TurboQuant), 2~Bit (TurboQuant), 1~Bit (binär)
...
\begin{itemize}
  ...
  \item \textbf{PCA, erklärte Varianz}, also die Anzahl der Hauptkomponenten, die 95\% der Gesamtvarianz erklären. Dieses Maß erfasst die globale lineare Dimensionalität.
\end{itemize}
%
...
Für jedes Paar werden vier Distanzwerte berechnet, nämlich die Cosine-Similarity im Float-Raum, die jeweilige Distanz in den beiden TurboQuant-Räumen und die Hamming-Distanz im binären Raum.
%
...
Ein Verfahren mit hoher Korrelation, aber hohem MAE transformiert die Distanzen zwar monoton korrekt, verschiebt aber das gesamte Distanzniveau. Für Schwellwert-basierte Systeme wäre dies problematisch.
%
...
Für ein Retrieval-System ist es gravierender, irrelevante Dokumente anzuzeigen (niedriges Trustworthiness), als einzelne relevante Dokumente zu übersehen (niedrige Continuity), solange genügend relevante Treffer verbleiben. Ob Letzteres der Fall ist, überprüft der Recall in Phase~4 direkt.
%
...
%
Für jede Query im BEIR-Testset wird eine Suche in allen vier Repräsentationsräumen durchgeführt:
\begin{itemize}
  \item Float-Raum: Ranking nach Cosine-Similarity.
  \item TurboQuant 4-Bit: Ranking nach der entsprechenden Distanzfunktion.
  \item TurboQuant 2-Bit: Ranking nach der entsprechenden Distanzfunktion.
...
Die Konsequenz dieser Entscheidung ist, dass die gemessenen Latenzen nicht direkt auf Produktivsysteme übertragbar sind, in denen bei größeren Korpora approximative Indizes (z.\,B. HNSW) unvermeidbar werden.
...
Bei sechs paarweisen Vergleichen pro Metrik wird eine Bonferroni-Korrektur angewandt ($\alpha_\text{korr} = 0{,}008$), um das multiple Testproblem zu berücksichtigen.
%
...
\begin{itemize}
  ...
  \item TurboQuant 4-Bit: $768 \times 4 = 3\,072$ Bit $= 384$ Byte
  \item TurboQuant 2-Bit: $768 \times 2 = 1\,536$ Bit $= 192$ Byte
...
Gemessen werden:
\begin{itemize}
  \item \textbf{Indexierungszeit}, also die Zeit, um alle Dokumente in die jeweilige Repräsentation zu überführen und zu indexieren.
  \item \textbf{Query-Latenz}, die mittlere Zeit pro Query bei Brute-Force-Suche.
  \item \textbf{Throughput}, gemessen in Queries pro Sekunde.
\end{itemize}
%
...
Die Korpusgrößen liegen allerdings unterhalb der Schwelle, ab der Speicher- und Latenzvorteile komprimierter Repräsentationen ihre volle Wirkung entfalten.
Bei Korpora im Millionenbereich wächst der praktische Nutzen überproportional, da der gesamte Index nicht mehr in den CPU-Cache passt und Speicherbandbreite zum Engpass wird.
Die vorliegenden Messungen sind daher als konservative untere Abschätzung des Effizienzgewinns zu interpretieren.
%
...
Die in den vorherigen Phasen untersuchten Retrieval-Backends (Float, TurboQuant 4-Bit, TurboQuant 2-Bit, binär) werden als MCP-Server implementiert und von einem LLM-Agenten über das Model Context Protocol (vgl. Kapitel~\ref{sec:grundlagen_mcp}) als Retrieval-Werkzeug genutzt.
%
...
\begin{enumerate}
  \item \textbf{Qualitätsübertragung:} Die in Phase~4 gemessenen NDCG-Unterschiede zwischen den Repräsentationsräumen spiegeln sich in der Antwortqualität des Agenten wider. Ein Retrieval-Backend, das im BEIR-Benchmark signifikant schlechter abschneidet, liefert dem Agenten schlechtere Kontextdokumente, was sich in der Antwortqualität niederschlägt.
  \item \textbf{Latenzvorteil:} Die in Phase~5 gemessenen Geschwindigkeitsunterschiede manifestieren sich als wahrnehmbarer Unterschied in der End-to-End-Antwortzeit des Agenten. Insbesondere binäre Repräsentationen sollten die Tool-Call-Latenz messbar senken.
\end{enumerate}
%
...
\begin{itemize}
  \item Vier identische MCP-Server, die sich ausschließlich im Retrieval-Backend unterscheiden (Float/Cosine, TurboQuant 4-Bit, TurboQuant 2-Bit, Binär/Hamming).
...
Der Agent, das Prompt-Template und die Dokumentenbasis bleiben über alle vier Varianten konstant.
%
...
Für jede Query wird der Agent mit jedem der vier Backends ausgeführt.
Gemessen werden:
\begin{itemize}
  ...
  \item \textbf{Retrieval-Übereinstimmung:} Der Overlap zwischen den vom MCP-Server zurückgegebenen Dokumenten und den Top-$k$-Ergebnissen der exakten Suche aus Phase~4. Dieser Wert quantifiziert, ob die MCP-Integration selbst (Serialisierung, Query-Weiterleitung) die Ergebnisse verändert.
  \item \textbf{Antwortqualität:} Bewertet wird, ob der Agent auf Basis der zurückgegebenen Dokumente korrekte und vollständige Antworten generiert. Eine automatisierte Bewertung über LLM-as-Judge \cite{zheng_2023_llmjudge} ergänzt die manuelle Stichprobe.
\end{itemize}
%
...
Die Latenzdifferenz zwischen Float und Binär sollte sich bei den verwendeten Korpusgrößen (5\,000 bis 171\,000 Dokumente) im einstelligen Millisekundenbereich bewegen.
Ob dieser Unterschied im Kontext der deutlich höheren LLM-Generierungslatenz praktisch relevant ist, wird quantifiziert.
Falls das binäre Backend bei bestimmten Queries schlechtere Dokumente liefert (gemäß Phase~4), sollte sich dies in der Antwortqualität niederschlagen.
